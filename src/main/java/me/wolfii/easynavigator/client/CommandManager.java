...
public class CommandManager {
    public static void registerCommand(CommandDispatcher<FabricClientCommandSource> dispatcher, CommandBuildContext ignoredRegistryAccess) {
        final LiteralCommandNode<FabricClientCommandSource> navigateNode = dispatcher.register(ClientCommands.literal("navigate")
            .then(ClientCommands.argument("blockpos", new NavigationArgumentType())
                .executes((context) -> {
                    ...
                    CommandSourceStack serverCommandSource = new CommandSourceStack(null, commandSource.getPosition(), commandSource.getRotation(), null, PermissionSet.ALL_PERMISSIONS, null, commandSource.getEntity());
                    ...
                    if (Minecraft.getInstance().player != null) {
                        ...
                        Vec3 targetPos = new Vec3(blockPos.getX() + 0.5, 0, blockPos.getZ() + 0.5);
        ...
        dispatcher.register(ClientCommands.literal("easynavigator:navigate").redirect(navigateNode));
        final LiteralCommandNode<FabricClientCommandSource> navigationNode = dispatcher.register(ClientCommands.literal("navigation")
        ...
        dispatcher.register(ClientCommands.literal("easynavigator:navigation").redirect(navigationNode));
    }
